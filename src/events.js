/**
 * MemoryVault Event Handlers
 *
 * Handles all SillyTavern event subscriptions and processing.
 */

import { getDeps } from './deps.js';
import { getOpenVaultData, getCurrentChatId, showToast, safeSetExtensionPrompt, withTimeout, log, isAutomaticMode, pruneMemoriesForBranch, saveOpenVaultData } from './utils.js';
...
import { autoHideOldMessages, unhideMessagesForBranch, scheduleAutoHideAfterBranchSwitch } from './auto-hide.js';
...
export async function onBeforeGeneration(type, options, dryRun = false) {
    ...
    try {
        ...
        showToast('info', 'Retrieving memories...', 'MemoryVault', { timeOut: 2000 });
        ...
        getDeps().console.error('MemoryVault: Error during pre-generation retrieval:', error);
        setStatus('error');
...
export async function onChatChanged() {
    ...
    const branchPruningEnabled = settings?.branchPruningEnabled !== false; // Default to true

    let dataChanged = false;

    if (branchPruningEnabled) {
        // Prune memories that don't belong in this branch
        // This handles the case when user creates a branch from an earlier message -
        ...
        // Unhide messages that were hidden but should now be visible in this branch
        const unhideResult = await unhideMessagesForBranch();
        if (unhideResult.unhiddenCount > 0) {
            showToast('info', `Unhid ${unhideResult.unhiddenCount} messages for branch`, 'MemoryVault');
            dataChanged = true;
        }

        // Schedule auto-hide to re-apply based on new chat length
        scheduleAutoHideAfterBranchSwitch();
    }
...
export async function onMessageReceived(messageId) {
    ...
    try {
        ...
        const data = getOpenVaultData();
        if (!data) {
            log('Cannot get MemoryVault data, skipping extraction');
            return;
        }
        ...
        showToast('info', `Extracting memories (messages ${batchToExtract[0] + 1}-${batchToExtract[batchToExtract.length - 1] + 1})...`, 'MemoryVault', {
            timeOut: 0,
            extendedTimeOut: 0,
            tapToDismiss: false,
            toastClass: 'toast memoryvault-extracting-toast'
        });
        ...
        $('.memoryvault-extracting-toast').remove();
        if (result && result.status === 'success' && result.events_created > 0) {
            showToast('success', `Extracted ${result.events_created} events from ${result.messages_processed} messages`, 'MemoryVault');
            refreshAllUI();
        }
    } catch (error) {
        getDeps().console.error('[MemoryVault] Automatic extraction error:', error);
        // Clear the persistent toast and show error
        $('.memoryvault-extracting-toast').remove();
        showToast('error', `Extraction failed: ${error.message}`, 'MemoryVault');
    } finally {
