...
export async function extractAllMessages(updateEventListenersFn) {
    ...
    $(toastr?.info(
        ...
        'MemoryVault - Extracting',
        {
            timeOut: 0,
            extendedTimeOut: 0,
            tapToDismiss: false,
            toastClass: 'toast memoryvault-backfill-toast'
        }
    ));
    ...
    for (let i = 0; i < completeBatches; i++) {
        ...
        $('.memoryvault-backfill-toast .toast-message').text(
        ...
        try {
            ...
            if (error.message === 'Chat changed during extraction') {
                ...
                $('.memoryvault-backfill-toast').remove();
                showToast('warning', 'Backfill aborted: chat changed', 'MemoryVault');
                clearAllLocks();
                setStatus('ready');
                return;
            }
            console.error('[MemoryVault] Batch extraction error:', error);
            $('.memoryvault-backfill-toast .toast-message').text(
