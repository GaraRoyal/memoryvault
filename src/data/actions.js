/**
 * MemoryVault Data Actions
 *
 * Data manipulation layer for MemoryVault.
 * This module handles all mutations to MemoryVault data, keeping UI components decoupled from persistence logic.
 */
 ...
 * @returns {Promise<boolean>} True if toggled, false otherwise
 */
export async function toggleMemoryPin(id) {
    ...
    await getDeps().saveChatConditional();
    log(`${memory.pinned ? 'Pinned' : 'Unpinned'} memory ${id}`);
    return true;
}
