 ...
 * Stage 0: Separate pinned memories (always included)
...
export async function selectRelevantMemories(memories, recentContext, userMessages, characterName, activeCharacters, settings, chatLength) {
    ...
    // Stage 0: Separate pinned memories (always included regardless of scoring)
    const pinnedMemories = memories.filter(m => m.pinned === true);
    const unpinnedMemories = memories.filter(m => m.pinned !== true);
    const pinnedTokens = pinnedMemories.reduce((sum, m) => sum + estimateTokens(m.summary), 0);

    if (pinnedMemories.length > 0) {
        log(`Stage 0: ${pinnedMemories.length} pinned memories (~${pinnedTokens} tokens) will be included`);
    }

    // Adjust token budgets for remaining memories
    const adjustedPreFilterTokens = Math.max(0, preFilterTokens - pinnedTokens);
    const adjustedFinalTokens = Math.max(0, finalTokens - pinnedTokens);

    // If only pinned memories exist, return them
    if (unpinnedMemories.length === 0) {
        return pinnedMemories;
    }

    // Stage 1: Algorithmic Filtering (on unpinned memories only)
    ...
    const stage1Results = sliceToTokenBudget(scored, adjustedPreFilterTokens);
    log(`Stage 1: ${unpinnedMemories.length} memories -> ${scored.length} scored -> ${stage1Results.length} after token filter (${adjustedPreFilterTokens} budget)`);
    ...
    if (stage1Results.length > 0) {
        if (settings.smartRetrievalEnabled) {
            ...
            const targetCount = Math.max(1, Math.floor(adjustedFinalTokens / avgMemoryCost));
            ...
            if (stage1Results.length <= targetCount) {
                ...
                finalResults = stage1Results;
            } else {
                finalResults = await selectRelevantMemoriesSmart(stage1Results, recentContext, userMessages, characterName, targetCount, chatLength);
            }
        } else {
            // Simple mode: just apply final token budget
            finalResults = sliceToTokenBudget(stage1Results, adjustedFinalTokens);
            log(`Stage 2 (Simple): ${stage1Results.length} -> ${finalResults.length} after final token filter (${adjustedFinalTokens} budget)`);
        }
    }

    // Combine pinned memories (first) with selected memories
    const combined = [...pinnedMemories, ...finalResults];
    if (pinnedMemories.length > 0) {
        log(`Final: ${pinnedMemories.length} pinned + ${finalResults.length} scored = ${combined.length} total memories`);
    }
