...
export function safeParseJSON(input) {
    try {
        ...
        if (parsed === null || typeof parsed !== 'object') {
            getDeps().console.error('[MemoryVault] JSON Parse returned non-object/array:', typeof parsed);
            getDeps().console.error('[MemoryVault] Raw LLM response:', input);
            return null;
        }
        return parsed;
    } catch (e) {
        getDeps().console.error('[MemoryVault] JSON Parse failed', e);
        getDeps().console.error('[MemoryVault] Raw LLM response:', input);
        return null;
    }
}
